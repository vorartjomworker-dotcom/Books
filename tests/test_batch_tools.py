...
from scripts.validate_batch import missing_source_tokens, validate_batch
...
class BatchToolsTests(unittest.TestCase):
    ...
    def test_localized_numbers_are_equivalent(self):
        self.assertEqual(
            missing_source_tokens(
                "AUC 0.87, 125,000 items, 13%, file.py",
                "AUC 0,87; 125 000 элементов; 13%; file.py",
            ),
            [],
        )
        self.assertEqual(missing_source_tokens("range [0,1]", "диапазон [0, 1]"), [])

    def test_qa_pass_requires_target_html(self):
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory); registry = self.make_registry(root); batch = root / "batch.csv"
            header, records = read_registry(registry); write_csv(batch, header, select_range(records, "CH05", 1, 1))
            updates = {"CH05-B0001": {"russian_text": "Значение 1% file.py", "translation_status": "QA_PASS"}}
            update_path = root / "updates.json"; update_path.write_text(json.dumps(updates, ensure_ascii=False), encoding="utf-8")
            output = root / "translated.csv"; apply_updates(batch, update_path, output)
            errors = validate_batch(registry, output, "CH05", 1, 1)
            self.assertIn("CH05-B0001: QA_PASS requires target_html", errors)
